...
static struct trig aw8622x_trig1 = {// 配置trig1引脚低高平触发1号波形
	.trig_brk = 0,
	.trig_level = 1,
	.trig_polar = 0,
	.pos_enable = 1,
...
struct aw_haptic_dts_info aw86214_dts = {
	.f0_pre = 2350,
	.f0_cali_percent = 7,
	.cont_drv1_lvl = 0x7F,
...
static void aw862xx_play_stop(void)
{
	...
	haptic_nv.play_mode = AW_STANDBY_MODE;
...
#ifdef AW86214_DRIVER
static void aw86214_config(void)
{
	uint8_t val = 0;

	AW_LOGI("enter");
	aw862xx_sram_size(AW862XX_HAPTIC_SRAM_1K);
	aw862xx_auto_brk_config(AW_TRUE);
	haptic_nv_i2c_writes(AW862XX_REG_CONTCFG10, &val, AW_I2C_BYTE_ONE);
}

static void aw86214_brk_en(aw_bool is_enable)
{
	uint8_t reg_val = 0;

	AW_LOGI("enter");
	if (is_enable == AW_FALSE)
		reg_val = 0x00;
	else
		reg_val = (uint8_t)haptic_nv.info->cont_brk_time;
	haptic_nv_i2c_writes(AW862XX_REG_CONTCFG10, &reg_val, AW_I2C_BYTE_ONE);
}

static int aw86214_play_mode_config(uint8_t play_mode)
{
	int ret = AW_SUCCESS;
	...
	case AW_RAM_LOOP_MODE:
		aw86214_brk_en(AW_FALSE);
		break;
	case AW_CONT_MODE:
		aw86214_brk_en(AW_TRUE);
		break;
	case AW_RTP_MODE:
	case AW_TRIG_MODE:
		ret = AW_ERROR;
...
static void aw862xx_play_mode(uint8_t play_mode)
{
...
#ifdef AW86214_DRIVER
	if (haptic_nv.name == AW86214) {
		if (aw86214_play_mode_config(play_mode) == AW_ERROR)
			return;
	}
#endif
	...
	case AW_RAM_MODE:
		...
		haptic_nv.play_mode = AW_RAM_MODE;
	...
	case AW_RAM_LOOP_MODE:
		...
		haptic_nv.play_mode = AW_RAM_LOOP_MODE;
		aw862xx_set_pwm(AW_PWM_12K);
		haptic_nv_i2c_write_bits(AW862XX_REG_PLAYCFG3, AW862XX_BIT_PLAYCFG3_PLAY_MODE_MASK,
								 AW862XX_BIT_PLAYCFG3_PLAY_MODE_RAM);
		break;
	case AW_CONT_MODE:
		...
		haptic_nv.play_mode = AW_CONT_MODE;
		haptic_nv_i2c_write_bits(AW862XX_REG_PLAYCFG3, AW862XX_BIT_PLAYCFG3_PLAY_MODE_MASK,
								 AW862XX_BIT_PLAYCFG3_PLAY_MODE_CONT);
		break;
	case AW_RTP_MODE:
		AW_LOGI("enter rtp mode");
		haptic_nv.play_mode = AW_RTP_MODE;
	...
	case AW_TRIG_MODE:
		...
		haptic_nv.play_mode = AW_TRIG_MODE;
		aw862xx_set_pwm(AW_PWM_12K);
		haptic_nv_i2c_write_bits(AW862XX_REG_PLAYCFG3, AW862XX_BIT_PLAYCFG3_PLAY_MODE_MASK,
								 AW862XX_BIT_PLAYCFG3_PLAY_MODE_RAM);
		break;
...
#ifdef AW8622X_DRIVER
static void aw8622x_config(void)
{
	uint8_t reg_val = 0;

	AW_LOGI("enter");
	aw862xx_sram_size(AW862XX_HAPTIC_SRAM_3K);
	aw862xx_auto_brk_config(haptic_nv.info->is_enabled_auto_brk);
	...
	if (haptic_nv.info->cont_brk_time) {
		reg_val = haptic_nv.info->cont_brk_time;
		haptic_nv_i2c_writes(AW862XX_REG_CONTCFG10, &reg_val, AW_I2C_BYTE_ONE);
	} else {
		AW_LOGE("dts_info->cont_brk_time=0");
	}
	if (haptic_nv.info->cont_brk_gain) {
		haptic_nv_i2c_write_bits(AW862XX_REG_CONTCFG5, AW862XX_BIT_CONTCFG5_BRK_GAIN_MASK,
							 haptic_nv.info->cont_brk_gain);
	} else {
...
static void aw862xx_misc_para_init(void)
{
	...
	/* SIN_H */
	array[0] = haptic_nv.info->sine_array[0];
	/* SIN_L */
	array[1] = haptic_nv.info->sine_array[1];
	/* COS_H */
	array[2] = haptic_nv.info->sine_array[2];
	/* COS_L */
	array[3] = haptic_nv.info->sine_array[3];
	haptic_nv_i2c_writes(AW862XX_REG_SYSCTRL3, array,  AW_I2C_BYTE_FOUR);
	array[0] = haptic_nv.info->cont_wait_num;
	haptic_nv_i2c_writes(AW862XX_REG_CONTCFG4, &array[0], AW_I2C_BYTE_ONE);
	if (!haptic_nv.info->d2s_gain) {
		AW_LOGE("dts_info->d2s_gain = 0!");
	} else {
		haptic_nv_i2c_write_bits(AW862XX_REG_SYSCTRL7, AW862XX_BIT_SYSCTRL7_D2S_GAIN_MASK,
								 haptic_nv.info->d2s_gain);
	}
	if (haptic_nv.info->cont_tset && haptic_nv.info->cont_bemf_set) {
		array[0] = (haptic_nv.info->cont_tset << 4) | haptic_nv.info->cont_bemf_set;
		haptic_nv_i2c_writes(AW862XX_REG_CONTCFG13, &array[0], AW_I2C_BYTE_ONE);
	} else {
		AW_LOGE("cont_tset = 0x%02X, cont_bemf_set = 0x%02X",
				haptic_nv.info->cont_tset, haptic_nv.info->cont_bemf_set);
	}

	haptic_nv.info->cont_drv2_lvl = AW862XX_DRV2_LVL_FORMULA(haptic_nv.info->f0_pre, haptic_nv.info->lra_vrms);
	AW_LOGI("lra_vrms=%d, cont_drv2_lvl=0x%02X", haptic_nv.info->lra_vrms,
			haptic_nv.info->cont_drv2_lvl);
	if (haptic_nv.info->cont_drv2_lvl > AW862XX_DRV2_LVL_MAX) {
		AW_LOGE("cont_drv2_lvl[0x%02X] is error, restore max vale[0x%02X]",
				haptic_nv.info->cont_drv2_lvl, AW862XX_DRV2_LVL_MAX);
		haptic_nv.info->cont_drv2_lvl = AW862XX_DRV2_LVL_MAX;
	}
...
static void aw862xx_offset_cali(void)
{
	uint8_t reg_val = 0;
	int  cont = 2000;
	...
	while (1) {
		haptic_nv_i2c_reads(AW862XX_REG_DETCFG2, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw862xx_read_lra_f0(void)
{
	...
	if (!f0_reg) {
		...
		haptic_nv.f0 = haptic_nv.info->f0_pre;
		return;
	}
	f0_tmp = AW862XX_F0_FORMULA(f0_reg);
	haptic_nv.f0 = (uint32_t)f0_tmp;
	AW_LOGI("lra_f0=%d", haptic_nv.f0);
}
...
static int aw862xx_cont_get_f0(void)
{
	...
	haptic_nv_i2c_write_bits(AW862XX_REG_CONTCFG6, AW862XX_BIT_CONTCFG6_DRV1_LVL_MASK,
			   haptic_nv.info->cont_drv1_lvl);
	val[0] = haptic_nv.info->cont_drv2_lvl;
	haptic_nv_i2c_writes(AW862XX_REG_CONTCFG7, val, AW_I2C_BYTE_ONE);
	val[0] = haptic_nv.info->cont_drv1_time;
	val[1] = haptic_nv.info->cont_drv2_time;
	haptic_nv_i2c_writes(AW862XX_REG_CONTCFG8, val, AW_I2C_BYTE_TWO);
	/* TRACK_MARGIN */
	if (!haptic_nv.info->cont_track_margin) {
		AW_LOGE("dts_info->cont_track_margin = 0!");
	} else {
		val[0] = haptic_nv.info->cont_track_margin;
		haptic_nv_i2c_writes(AW862XX_REG_CONTCFG11, &val[0], AW_I2C_BYTE_ONE);
	}
	/* DRV_WIDTH */
	if (!haptic_nv.info->f0_pre)
		return AW_ERROR;
	drv_width = AW862XX_DRV_WIDTH_FORMULA(haptic_nv.info->f0_pre,
										  haptic_nv.info->cont_track_margin,
										  haptic_nv.info->cont_brk_gain);
...
static void aw862xx_calculate_cali_data()
{
	...
	f0_cali_step = 100000 * ((int)haptic_nv.f0 - (int)haptic_nv.info->f0_pre) /
				   ((int)haptic_nv.f0 * AW862XX_F0_CALI_ACCURACY);
	...
	if (f0_cali_step > 31)
		...
		f0_cali_lra = (char)f0_cali_step + 32;
	/* update cali step */
	haptic_nv.f0_cali_data = (int)f0_cali_lra;
	AW_LOGI("f0_cali_data=0x%02X", haptic_nv.f0_cali_data);
}
...
static void aw862xx_set_base_addr(void)
{
	...
	base_addr = haptic_nv.ram.base_addr;
...
static void aw862xx_haptic_trig_param_init(uint8_t pin)
{
	switch (pin) {
	case AW_TRIG1:
		haptic_nv.trig[0] = &aw8622x_trig1;
		break;
	case AW_TRIG2:
		haptic_nv.trig[1] = &aw8622x_trig2;
		break;
	case AW_TRIG3:
		haptic_nv.trig[2] = &aw8622x_trig3;
		break;
	default:
		break;
	}
...
static int aw862xx_haptic_trig_param_config(uint8_t pin)
{
	...
	if ((haptic_nv.name == AW86224 ||haptic_nv.name == AW86225) &&
		 haptic_nv.is_used_irq_pin) {
	...
	case AW_TRIG1:
		...
		trig_polar_lev_brk = haptic_nv.trig[0]->trig_polar << 2 |
							 haptic_nv.trig[0]->trig_level << 1 |
							 haptic_nv.trig[0]->trig_brk;
		haptic_nv_i2c_write_bits(AW862XX_REG_TRGCFG7, AW862XX_BIT_TRGCFG7_TRG1_POR_LEV_BRK_MASK,
								 trig_polar_lev_brk << 5);
		trig_pos_seq = haptic_nv.trig[0]->pos_enable << 7 |
					   haptic_nv.trig[0]->pos_sequence;
		haptic_nv_i2c_writes(AW862XX_REG_TRGCFG1, &trig_pos_seq, AW_I2C_BYTE_ONE);
		trig_neg_seq = haptic_nv.trig[0]->neg_enable << 7 |
					   haptic_nv.trig[0]->neg_sequence;
		haptic_nv_i2c_writes(AW862XX_REG_TRGCFG4, &trig_neg_seq, AW_I2C_BYTE_ONE);
		AW_LOGI("trig1 config ok!");
		break;
	case AW_TRIG2:
		trig_polar_lev_brk = haptic_nv.trig[1]->trig_polar << 2 |
							 haptic_nv.trig[1]->trig_level << 1 |
							 haptic_nv.trig[1]->trig_brk;
		haptic_nv_i2c_write_bits(AW862XX_REG_TRGCFG7, AW862XX_BIT_TRGCFG7_TRG2_POR_LEV_BRK_MASK,
				   trig_polar_lev_brk << 1);
		trig_pos_seq = haptic_nv.trig[1]->pos_enable << 7 |
					   haptic_nv.trig[1]->pos_sequence;
		haptic_nv_i2c_writes(AW862XX_REG_TRGCFG2, &trig_pos_seq, AW_I2C_BYTE_ONE);
		trig_neg_seq = haptic_nv.trig[1]->neg_enable << 7 |
					   haptic_nv.trig[1]->neg_sequence;
		haptic_nv_i2c_writes(AW862XX_REG_TRGCFG5, &trig_neg_seq, AW_I2C_BYTE_ONE);
		AW_LOGI("trig2 config ok!");
		break;
	case AW_TRIG3:
		trig_polar_lev_brk = haptic_nv.trig[2]->trig_polar << 2 |
							 haptic_nv.trig[2]->trig_level << 1 |
							 haptic_nv.trig[2]->trig_brk;
		haptic_nv_i2c_write_bits(AW862XX_REG_TRGCFG8, AW862XX_BIT_TRGCFG8_TRG3_POR_LEV_BRK_MASK,
				   trig_polar_lev_brk << 5);
		trig_pos_seq = haptic_nv.trig[2]->pos_enable << 7 |
					   haptic_nv.trig[2]->pos_sequence;
		haptic_nv_i2c_writes(AW862XX_REG_TRGCFG3, &trig_pos_seq, AW_I2C_BYTE_ONE);
		trig_neg_seq = haptic_nv.trig[2]->neg_enable << 7 |
					   haptic_nv.trig[2]->neg_sequence;
...
static void aw862xx_get_vbat(void)
{
	...
	haptic_nv.vbat = AW862XX_VBAT_FORMULA(vbat_code);
	if (haptic_nv.vbat > AW_VBAT_MAX) {
		haptic_nv.vbat = AW_VBAT_MAX;
		AW_LOGI("vbat max limit = %dmV", haptic_nv.vbat);
	}
	if (haptic_nv.vbat < AW_VBAT_MIN) {
		haptic_nv.vbat = AW_VBAT_MIN;
		AW_LOGI("vbat min limit = %dmV", haptic_nv.vbat);
	}
	AW_LOGI("vbat=%dmV, vbat_code=0x%02X", haptic_nv.vbat, vbat_code);
	aw862xx_raminit(AW_FALSE);
}
...
static void aw862xx_get_lra_resistance(void)
{
	...
	haptic_nv_i2c_write_bits(AW862XX_REG_SYSCTRL7, AW862XX_BIT_SYSCTRL7_D2S_GAIN_MASK,
							 haptic_nv.info->d2s_gain);
	d2s_gain = aw862xx_select_d2s_gain(haptic_nv.info->d2s_gain);
	if (d2s_gain < 0) {
...
static void aw862xx_cont_config(void)
{
	...
	haptic_nv_i2c_write_bits(AW862XX_REG_CONTCFG6,
			   ...
			   (AW862XX_BIT_CONTCFG6_TRACK_ENABLE | haptic_nv.info->cont_drv1_lvl));
	reg_val = haptic_nv.info->cont_drv2_lvl;
